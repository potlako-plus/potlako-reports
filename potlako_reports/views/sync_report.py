...
class SyncReportView(TemplateView, NavbarViewMixin, EdcBaseViewMixin):
    ...
    @property
    def clinician_call_enrol_model_cls(self):
        return django_apps.get_model(self.clinician_call_enrol_model)
    ...
    def host_machines(self):
        ...
        if self.device_id:
            ...
            device_ids = set(host_names.values_list('device_created', flat=True))

        return sorted(device_ids)  # , sorted(host_names)
    ...
    def crf_statistics(self):
        ...
        for model_class_name in self.crf_models:
            ...
            if self.device_id:
                ...
                for host_machine in self.host_machines:
                    counter = model_class.objects.filter(
                        device_created=host_machine).count()
                    model_class_statistics.append(counter)
